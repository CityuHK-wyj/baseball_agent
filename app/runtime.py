...
def build_pipeline(*, runtime_dir: Path | None = None, knowledge=None, recorder=None,
    ...
    if semantic_parser is None:
        if semantic_extractor is not None:
            semantic_parser = HybridSemanticParser(extractor=semantic_extractor)
        elif llm_provider is not None:
            from app.persistence.semantic_review import SemanticReviewStore
            from app.semantic.dual_parser import build_dual_parser
            review_store = SemanticReviewStore(recorder.store) if recorder is not None else None
            semantic_parser = build_dual_parser(
                llm_provider,
                extractor_model=settings.semantic_extractor_model,
                reviewer_model=settings.semantic_reviewer_model,
