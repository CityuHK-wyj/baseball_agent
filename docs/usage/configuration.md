...
| `SEMANTIC_MODEL` | no | `DEEPSEEK_MODEL` | Fallback model for both semantic roles. |
| `SEMANTIC_EXTRACTOR_MODEL` | no | `SEMANTIC_MODEL` → fast default | Model for the semantic extractor (LLM A). |
| `SEMANTIC_REVIEWER_MODEL` | no | `SEMANTIC_MODEL` → fast default | Model for the independent semantic reviewer (LLM B). |
| `SEMANTIC_DEFAULT_MODEL` | no | `deepseek-chat` | Built-in fast default for the dual roles when no other model variable is set. |
| `LLM_TIMEOUT_SECONDS` | no | `30` | Per-request timeout. |

The dual semantic roles are independently configurable and use separate calls and prompts;
using one provider for both is supported. `DEEPSEEK_MODEL` still defaults to
`deepseek-v4-pro`; the semantic roles deliberately default to a faster model because the
review path makes two calls. No model name is hard-coded in domain logic — the provider is
injected through the `ModelProvider` seam.
